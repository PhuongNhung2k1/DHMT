...
mat4 instance, ctm_bangv;
GLdouble mauAnhSang = 1.25;
...
{
	toMau(82, 254, 255);
	instance = Translate(-3, 0, 0.0) * Scale(0.05,6,6);
...
{
	toMau(82, 254, 255);
	instance = Translate(0, 0, 3) * Scale(6, 6, 0.05);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void nen()
{
	toMau(82, 254, 255);
	instance = Translate(0.0, -3, 0.0) * Scale(6, 0.05, 6);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void tran()
...
GLfloat eyeX = 0.0, eyeZ = -3.5, X1 = 0, Z1 = 0;
GLfloat X4 = 0, Z4 = 0;
...
void display(void)
{
	...
	//projection = Frustum(-3, 3, -3, 3, 3, 32);
	

	projection = Frustum(-1.5,1, -1, 3.5, 1, 15);
